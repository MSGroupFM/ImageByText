...
function getSizeByText($font_size, $font_angle, $font_file, $text)
{
 ...
 */
function makeImageByText(&$image, $size, $angle, $left, $top, $color, $font, $text, $align, $width, $height)
{
    $text_lines = explode("\n", $text);
    ...
    else
    {
        ...
        {
            $dimensions = getSizeByText($size, $angle, $font, $block);
...
$inText = isset($_GET['text']) ? (string)trim($_GET['text']) : "Картинка из текста"; // Текст

$redColor = isset($_GET['red']) ? intval($_GET['red']) : 0;
$greenColor = isset($_GET['green']) ? intval($_GET['green']) : 0; // Цвет текста в формате RGB (по умолчанию черный)
$blueColor = isset($_GET['blue']) ? intval($_GET['blue']) : 0;

$inFontFile = isset($_GET['font']) ? trim($_GET['font']) : "Arial.ttf"; // Шрифт для текста
$inFontSize = isset($_GET['fsize']) ? trim($_GET['fsize']) : 24; // Размер шрифта в типографских пунктах.
$inAlign = isset($_GET['align']) ? trim($_GET['align']) : ALIGN_CENTER; // Выравнивание текста (left, right, center)
$inDeg = isset($_GET['deg']) ? trim($_GET['deg']) : 0; // Угол поворота итогового изображения

/**Расчет размера изображения на основе текста*/
...
makeImageByText($img, $inFontSize, 0, $x, $y, $text_colour, $inFontFile, $inText, $inAlign, $width, $height);
$img = imagerotate($img, $inDeg, 0); // Поворот изображения
